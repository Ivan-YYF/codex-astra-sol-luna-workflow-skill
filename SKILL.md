...
Keep the title stable across ordinary follow-ups, waits, corrections, and effort changes. `[Luna]` identifies the workflow role; do not put the model version, `high`, `max`, runtime state, task identifiers, paths, or other configuration in the title. Do not create a replacement task merely to refresh a title after the root task is renamed.
