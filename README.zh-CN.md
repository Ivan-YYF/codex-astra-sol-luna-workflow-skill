...
如果用户提供精确的项目目录（例如 `D:\codex\Promotion OS`），或要求使用当前项目，该路径就是权威执行位置。如果用户没有提供精确地址，则使用根任务当前已核验的项目目录（或用户明确选择的其他已保存项目）作为目标。两种情况下都请求直接 `local` 执行，并在 Luna 写入前核对新任务的实际路径。若返回的是 `worktree` 或其他目录，则视为创建失败：停止并报告路径不一致，不得静默改道或自动创建替代任务。只有用户明确要求隔离时，才使用隔离的 `worktree`。
...
如果为精确路径或当前项目默认选择了直接 `local` 执行，回执中应单独增加 **Path** 字段，显示解析出的绝对目录，并在设置证据到达前标记为待核验；在后续进度或完成回执中报告实际核验路径。若返回的是 `worktree` 或其他目录，应视为创建失败，而不是在回执中隐藏位置差异。
...
- 根任务解析出的当前项目路径即使在用户未提供精确地址时也是执行位置硬约束：除非用户明确要求隔离，否则使用直接 `local` 执行，并在任何写入前验证任务实际路径。
